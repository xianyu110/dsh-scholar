 ...
 *   `DSH_SCHOLAR_BRIDGE_TOKEN` or `ResearchUiBridgeOptions.token`), every
 *   request must present `Authorization: Bearer <token>` or
 *   `X-Research-Token`; otherwise 401. Opt-in (default off). The token is
 *   only read from env/config, never logged, never in argv.
 * - Session bootstrap: GET /research-ui-api/session-token hands the token to
 *   the same-origin browser client (Sec-Fetch-Site / Origin guarded).
 * - CSRF: non-GET requests with an `Origin` header are accepted only when
 *   the origin is 127.0.0.1/localhost on the request's own port; 403.
 * - Body limit: request bodies capped at 16 MiB; beyond that 413.
 * - Binary streaming: JSON upstream responses forward as text; any other
 *   content type streams through as bytes (artifacts, PDFs, images) with
 *   content-type/content-length/content-disposition kept.
 * - Error desanitization: upstream failures return code + generic message
 *   only; internal paths/env never leak.
...
const API_PREFIX = '/research-ui-api'
const SESSION_TOKEN_PATH = '/research-ui-api/session-token'
const ALLOWED_ORIGIN_HOSTS = new Set(['127.0.0.1', 'localhost', '[::1]'])

export interface ResearchUiBridgeOptions {
...
/** CSRF origin check: same-site only for 127.0.0.1/localhost on the Host port. */
...
/** Read the request body up to `limit` bytes; oversized bodies are flagged. */
...
/**
 * Session bootstrap: hands the bridge token to the same-origin browser
 * client. 404 when token mode is off. Cross-site reads are rejected via
 * Sec-Fetch-Site / Origin; the response is no-store + nosniff.
 */
function handleSessionToken(req: IncomingMessage, res: ServerResponse, token: string | undefined): void {
  if (token === undefined) {
    sendError(res, 404, 'not_found', 'session token endpoint is disabled')
    return
  }
  const fetchSite = req.headers['sec-fetch-site']
  if (fetchSite !== undefined && fetchSite !== 'same-origin' && fetchSite !== 'none') {
    sendError(res, 403, 'forbidden', 'cross-site token request rejected')
    return
  }
  const origin = req.headers.origin
  if (typeof origin === 'string' && !isAllowedOrigin(origin, req.headers.host)) {
    sendError(res, 403, 'forbidden', 'cross-origin token request rejected')
    return
  }
  res.writeHead(200, {
    'content-type': 'application/json; charset=utf-8',
...
export function registerResearchUiBridge(ctx: Context, sidecar: UiKernelSidecar, options: ResearchUiBridgeOptions = {}): void {
    ...
    httpCtx.effect(() => httpCtx.httpServer.register({
      ...
      handler: async (req: IncomingMessage, res: ServerResponse): Promise<void> => {
        try {
          ...
          // 2. Session bootstrap for the same-origin browser client.
          if (url.pathname === SESSION_TOKEN_PATH) {
            handleSessionToken(req, res, token)
            return
          }
          // 3. Token auth (opt-in; §15.3).
          ...
          // 4. CSRF: origin check for state-changing requests (§15.2).
          ...
          // 5. Strip the bridge prefix; the rest is the kernel path (/v1/...).
          const target = `${sidecar.endpoint}${url.pathname.replace(/^\/research-ui-api/, '')}${url.search}`
          // 6. Read the body under the 16 MiB cap.
          ...
          // 7. Proxy to the kernel. In token mode the same token authenticates
