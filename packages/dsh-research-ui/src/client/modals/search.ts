...
export function openNotificationsModal(root: ShadowRoot | null | undefined): void {
  ...
  const header = el('div', 'modal-header', `🔔 Notifications (${state.notifHistory.length})`)
...
export function openCommandHistoryModal(root: ShadowRoot | null | undefined): void {
  ...
  const hint = el('div', 'muted', `${state.chatHistory.length} commands · click one to re-run it in Chat (↑/↓ also walk this list in the composer).`)
...
export function openSessionSearchModal(root: ShadowRoot | null | undefined): void {
  ...
  const runSearch = (): void => {
    ...
    if (hits.length === 0) {
      results.appendChild(el('div', 'empty', `No matches for "${sessionSearchQuery.trim()}" across ${state.chatSessions.length} session(s).`))
      return
    }
    const count = el('div', 'muted', `${hits.length} hit(s) across ${state.chatSessions.length} session(s) — ↑/↓ + Enter to jump.`)
    count.style.cssText = 'margin-bottom:8px;font-size:11px'
    results.appendChild(count)
