...
Standalone Chat 的普通文本必须先读取当前 project projection，再根据自然语言与 `next_actions_v2` 回答当前阶段和下一步。状态、想法、Gate、Jobs 与 Claims 走只读 canonical operation；带 query 且 ready 的调研以及 ready 的无参数写作、审阅、发布包动作可由用户明确要求直接执行；实验/复现等参数不完整动作只生成可编辑命令。模型或自然语言解析绝不能执行 Gate Decision、Brief confirm、Intake adoption 或 Release Decision，也不能扩大 ready/capability/ACL。没有模型 adapter 时仍返回包含当前阶段、next action 和阻断原因的确定性回答，不得出现 `Unknown command` 或要求用户先输入 slash。
