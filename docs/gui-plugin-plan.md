...
完整科研浏览器 UI 的实现、HTTP host 与 BFF 只有 standalone 一份：本地 HTTP host、同源 BFF、全屏 UI、独立 Token 解锁，并内置 locale/theme adapter。DSH Agent 插件继续提供 tools、commands、Skills 和 Settings → Plugin config 配置卡，同时在会话 `Chat`、`Trajectory` 之后注册 `dsh Scholar` 页签。该页签在 iframe 外渲染一个只读、同 session 的 Kernel 阶段投影，并以受控 iframe 展示已配置的 standalone URL、提供“在新页面打开”；它不是第二套业务 UI/BFF，不发布 legacy `dshClient`、不恢复 `/research-api` 或 `/research-ui-api` 桥，也不复制可变科研状态模型。
...
- standalone 响应只允许 `standalone.frame_ancestors` 配置的精确 HTTPS 或 loopback-HTTP DSH origin 嵌入（默认 `127.0.0.1/localhost/[::1]:3080`）；远端明文 HTTP 与非白名单 ancestor 由无通配符 CSP 拒绝。页面内联 bootstrap script/style 使用每响应 nonce，业务脚本只允许 self；iframe 使用最小 sandbox/referrer policy，不把 Token 放入 URL、postMessage、DOM attribute 或 boot manifest；
- standalone bootstrap 与宿主以 exact iframe source、exact origin 和 `protocol=1` 完成 ready/query 握手；宿主 8 秒内未收到 ready、收到 iframe error 或遇到 TLS/CSP/网络错误时必须卸载坏 iframe，显示本地化失败说明、重试、“在新页面打开”和非默认地址的恢复入口。握手只表示页面安全加载，不携带 Token、项目数据或用户输入；
...
同样的自然语言体验必须覆盖 DSH 本体 Chat，而不是只存在于 standalone composer。`research-core` Skill 指示 Harness Agent 在用户表达研究意图时调用单一 `dsh_scholar` façade，并原样传递文本；工具按当前 DSH session 绑定项目、执行受控 intent、返回本地化回答与最新阶段。Agent 不应要求用户先手写 slash；当动作不能安全自动执行时，回答要帮助生成可编辑的一级 slash command，并解释所需 Human/Agent/Runner 与阻断项。

DSH 页签中的 standalone iframe 通过 request-id 关联的 `postMessage` 请求 Host browser client，再由 trusted-host `/dsh-scholar-view` RPC 使用 Harness `ctx.llm` 完成无工具的一次对话调用；该 view channel 只暴露 `chat-turn` 与脱敏 `session-projection`，配置/Token 仍在独立的 loopback-only `/dsh-scholar`。模型桥仅对与 Host 不同 origin 的 loopback standalone，或同主机、不同端口/origin 的 HTTPS standalone 开放；跨主机 HTTPS、远端明文 HTTP 与 Host 同 origin iframe 只能使用 deterministic fallback。standalone HTML 用 `frame-ancestors` 只允许配置的精确 HTTPS 或 loopback HTTP embedder，iframe 使用 origin-only referrer；子页在发送任何 project/history 前必须从浏览器 `document.referrer` 确认父 origin 符合上述关系。父页面必须同时校验 iframe `contentWindow` 与配置的 standalone origin，已完成 request-id 在有界 128 项窗口内仍拒绝重放，且每个 iframe 最多一个 in-flight turn；子页从可信 referrer 固定 exact parent origin，以此作为 `postMessage` targetOrigin，并只接受 exact `window.parent`、origin 与 request-id 匹配的响应，禁止 `'*'`。父子两端 20 秒超时，iframe 失败态/卸载、URL 切换、同 URL 重试或 RPC 取消必须通过 `AbortSignal` 终止旧 Host LLM；不可用时立即回退确定性回答。请求最多携带最近 12 条、每条有界且在首次 await 前冻结的当前 project/session 文本以及裁剪后的 `project.status/brief_status/next_actions_v2`，禁止跨项目 transcript、附件字节和任何 secret。模型返回的候选 slash 必须由 direct descriptor allowlist 校验并排除 Human-only command，只渲染为“使用命令”按钮；点击仅把命令写入 composer 并把光标置于末尾，不自动发送。
