...
**STORE-08 已落地（0003 checksum 绑定，storage-migrations.md §8.1）**：0003 的 canonical body 从“函数源码”改为“函数源码 + 冻结的内联 DDL 快照文本”（`TERMINAL_DDL_0003`/`TEX_DDL_0003`，迁移执行的就是这两份冻结常量，不再引用共享 `TERMINAL_DDL`/`TEX_DDL`）。效果：checksum 绑定 0003 实际执行的 DDL——共享常量演进（新能力走新迁移 + tex-workspace/pty store 自带 CREATE IF NOT EXISTS 收敛）不会静默改变已发布迁移的行为，也不会使既有库上已记录的 checksum 失效；而任何对 0003 自身文本的改动都会 checksum mismatch loud fail（“released migration 不可修改”由 checksum 真正兜底）。验证：tests/unit/migrations.test.ts STORE-08 用例（body 为内联 DDL、不含共享常量引用、checksum 为 id+body 纯函数、对 body 的任何编辑都改变 checksum）。
