...
- `chat-grill-one-at-a-time`：每次 GET/answer 只暴露一个当前问题；revision stale 409；edit/skip/unknown 可恢复；刷新/重连从服务端继续；OCR 候选带 source/page/confidence 且不自动成为 human_assertion。**代码侧已闭环（2026-08-12）**：确定性状态机（`PROJECT_GRILL_QUESTIONS` 固定 7 题顺序 + question_revision=1 + required），answer 记录 Human Principal/human_assertion，`/new`、`/confirm-brief` 一级 slash command，消息只保存 attachment/stage ref；OCR 请求面（`/v2/intakes/{id}/ocr-requests`）按 init-grill-upload-models.md §5 契约实现记录为 `NOT_RUN_MANUAL_PENDING`（真实模型服务未接）。证据：kernel.test.ts Grill 顺序/确认用例、chat-command.test.ts、intake.test.ts（intake taxonomy 确定性）。
...
- `provider-secretref-ssrf`：global Provider CRUD revision CAS；严格 SecretRef 拒绝 value/token/password；列表/日志/Trajectory/Manifest 零 secret；非法 scheme、redirect、DNS rebinding/proxy 越界 fail closed；项目只能提交 provider/model ID。**代码侧已闭环（2026-08-12，MODEL-01）**：`/v1/providers*` CRUD + revision CAS（409 provider_revision_conflict）、SecretRef 严格 schema（value/token/password/credential 字段 422 secret_value_forbidden，本 kernel 从不存储 value）、base URL 服务端校验（scheme https-only（loopback 白名单 http）、userinfo 拒绝、私有/保留 IP 与未 allowlist DNS 主机 SSRF fail closed）、浏览器响应 credential 只回显 metadata+available（零 secret 断言）、项目绑定只收 opaque provider/model ID（provider 存在+enabled、模型在目录、能力匹配、快照 provider_revision+config_hash、绑定 revision CAS、删 provider 清绑定）。连接期 redirect/DNS-rebinding 复检由尚未实现的模型客户端承担（如实记录）。证据：tests/unit/provider-registry.test.ts 12/12。真实私有 Provider/模型服务调用记 `NOT_RUN_MANUAL_PENDING`。
