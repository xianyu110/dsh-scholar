...
- `init-name-only-no-gate`：POST /v2/projects 仅 name，返回 DRAFT/collecting + active Intake + membership/budget，Gate 数为 0；幂等重放同对象，异 hash 409；collecting projection 不出现 scope gate CTA，orchestrator 不自动补 Gate。
- `chat-grill-one-at-a-time`：每次 GET/answer 只暴露一个当前问题；revision stale 409；edit/skip/unknown 可恢复；刷新/重连从服务端继续；OCR 候选带 source/page/confidence 且不自动成为 human_assertion。
- `grill-confirm-scope-gate`：必答项处理后返回 Brief preview；非 PI/Agent confirm 403 或无工具；PI 使用 expected revisions 确认后写 confirmed Brief 与唯一 Scope Gate；confirm 重放不重复 Gate。
- `intake-batch-chunk-resume`：多文件队列按 8 MiB chunk；暂停/刷新/重连从 committed offset 继续；相同范围同 hash 重放成功，gap/不同内容/total 变化 409；finalize 重算完整 hash/size，失败零 IntakeArtifact；2 GiB 默认与 10 GiB 硬上限、并发预留不能绕过。
- `provider-secretref-ssrf`：global Provider CRUD revision CAS；严格 SecretRef 拒绝 value/token/password；列表/日志/Trajectory/Manifest 零 secret；非法 scheme、redirect、DNS rebinding/proxy 越界 fail closed；项目只能提交 provider/model ID。
...
- `init-model-upload-i18n`：上述所有 chrome、错误、aria 与下一步提示 zh/en parity；运行时切换不丢项目名、答案或队列。真实浏览器、2–10 GiB、私有 Provider 和 OCR 服务均记录 `NOT_RUN_MANUAL_PENDING`，通过前 hardening 只能写“已实现未验收”。

## 23. 论文复现、一级 slash command、Chat 附件、环境与 Session Terminal

- `slash-command-direct-only`：帮助/补全/descriptor 只提供 `/new`、`/reproduce`、`/confirm-brief` 等一级命令；DSH command list 不注册聚合 descriptor，standalone 也不解析或兼容旧聚合前缀；旧形式必须返回 unknown command，且不得进入 starter、Tab 补全、帮助、文档示例或 provenance；命令 description 在 zh/en 下按 locale key 即时求值，切换语言后不得残留冻结英文。
- `chat-attachment-intake-only`：按钮/拖拽/粘贴多文件进入当前 active Intake 分块队列；消息只含 attachment ref；scan/adoption 前 Project Artifact/CodeSnapshot/Job/Evidence 零写；刷新恢复队列，zh/en/aria 完整。
- `reproduction-spec-provenance`：DOI/arXiv/PDF、CodeSnapshot exact commit/submodules、data/license/split/preprocess、claims/metric locator、Contract 与 Principal 全部固定；raw repo URL 不能代替 Snapshot。
- `reproduction-environment-pinned`：Spec/attempt/ExecutionPlan/RunManifest 固定 profile/target/environment revision/hash；local Docker 与 remote SSH 使用同一接口；客户端 endpoint/SSH credential/ProxyCommand 拒绝；unknown/offline/draining/cap mismatch 无本机回退。
- `reproduction-comparison-report`：expected=0、absolute/relative、NaN/Infinity、unit/direction/aggregation、missing/duplicate metric、table row/column、figure data hash、manuscript TeX/PDF required/negative；exit 0 但 out-of-tolerance 报告 fail/inconclusive，不把 NextAction 标 done。
- `reproduction-attempt-report-fencing`：Spec revision、attempt idempotency、Job/Run/lease generation、signed RunManifest、verifier identity、immutable Report 与跨项目 AuthZ；Agent 无 report accept/Gate Decision。
