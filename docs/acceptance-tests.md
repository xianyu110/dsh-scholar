...
- configurable-execution-environments：Runner Target/Profile Registry 可经权威 API 与 Settings 创建/编辑/禁用 `local-process`、`local-docker`、`remote-ssh`；当前项目默认 target 可在 Settings 选择并以 `expected_revision` CAS 保存，禁用/排空/未知 target、stale revision、非 PI/operator 写入均拒绝；目标选项格式与 unknown/online/offline health 枚举必须走 zh/en 字典，不把 wire 英文直接放进 chrome；`/run`、`/reproduce` JSON 的顶层 `runner_target_id` 作为单次 Job 覆盖，断言请求不只把它塞进业务 payload，并验证“Job 覆盖 > 项目默认”。project/job/reproduction 仅引用 opaque IDs 并固定 target/profile/environment revision+hash。`local-process` 对正式 kind 拒绝；Docker 安全参数与 digest pin 保持；remote-ssh 的 endpoint、known-host/CA、credential 仅以服务端 SecretRef 存在且 safe view/日志/Job/ExecutionPlan 零泄漏；Settings 对三个 SecretRef 均分别提供 `scheme`、`name`、可选 `version`、可选 `scope` 控件，创建 payload 省略空可选字段，编辑往返必须逐字段保真且不得回显 secret value。unknown/offline/draining/capability/host-key mismatch 保持 blocked/retryable，绝不本机 fallback；revision CAS、权限、撤权、轮换和新旧 attempt pin 均有负向测试。**2026-08-12 自动侧已实现未验收**：0023 registry/API/Settings、SecretRef/CAS、target-aware claim、ExecutionPlan pin、Local/Fleet/Agent exact-target gate、受控 SSH bootstrap（strict endpoint、0600 key、pinned known_hosts、无 ProxyCommand/任意 command）；真实 SSH 主机、host-key 轮换与断网恢复按 manual 队列验收。
- runner-target-exact-dispatch：本地 runner 必须用显式 `--target-id`（未给时按 mode 选择内置 local-process/local-docker id）按 `runner_target_id` + kind 双重 claim，并在 spawn 前再次核对 exact id；同 kind 的 target A 不能执行 target B。claim 时重新读取当前 registry：disabled、draining、kind/revision/hash 漂移的 target/job 不得获得 lease；过滤后的 stale 候选不得占用 claim `limit` 并饿死其后的有效任务。claim 与真正 spawn 之间还必须再次从权威 registry 读取 target，校验 enabled、draining、kind、revision 与 config hash；任何漂移都以 environment failure 关闭当前 lease，绝不启动旧配置。remote online/offline 的执行权威来自认证 Agent Registry 的新鲜 heartbeat，持久 Target `health` 在 bridge 落地前仅为 UI 观测摘要——unknown 不得展示为 online，offline 摘要也不能覆盖更新鲜的认证 Agent 状态，二者不一致必须进入人工验收/告警且绝不本机回退。Target CAS 的条件更新必须检查 `changes===1`；项目创建与项目默认设置均在落库前校验 target 状态及 target/profile kind 兼容。
- reproduction-target-binding：论文复现 spec 的 `execution_binding.target_id` 必须解析为当前已登记、enabled、非 draining 且与 runner profile kind 兼容的 RunnerTarget；`environment_lock.target_id/revision/hash` 必须与 binding 和 registry 当前值一致，不能接受未知 target、伪造 revision/hash 或冲突 pin。启动 attempt 时重新解析并固化 target revision/hash；目标漂移后旧 spec/attempt 必须 fail closed，用户只能显式更新 spec 或创建新 attempt，禁止回退本机。
- remote-cas-resume：断点拉取/上传重试后 hash/size 一致，跨项目或 stale generation 拒绝。
- remote-plan-build-failure：Fleet 从 Kernel claim 后若 profile/target pin 校验或 ExecutionPlan 构建失败，必须用当前 generation/token 将该 lease 明确完成为 environment failure；不得把 Job 留在 running 直到 lease 过期，也不得加入 pending/claimed 去重集后静默阻塞。
...
- retention-gap：请求已淘汰 seq 先收到 gap 和 dropped bytes，随后至少收到 retained_from_seq 起的 retained frame；自动测试必须等待 gap 与 retained frame 两种事件都被 SSE parser 消费，不能把“已看到 gap”误当成同一网络 chunk 中后续 frame 已到达；
...
- 浏览器 TUI 验收：在真实浏览器打开 LocalPtyAdapter bash，聚焦终端输入 `printf 'WEBTERM_OK\\n'` 并看到输出；验证方向键历史、Tab、Ctrl+C、Unicode/粘贴、`vim`/`top` 或等价 alternate-screen TUI、自动 fit/resize、detach-reconnect、Dock right↔bottom、640/720/1024 视口。代码实现与 adapter 单测不能代替该项；未运行时记 `NOT_RUN_MANUAL_PENDING`。
...
- ui-chat-project-isolation：用项目 A/B 创建同名 Chat session、不同 transcript/active session/草稿/引用/附件，持久化并反复切换后每个项目只恢复自己的状态；storage key 与 `ChatSession.project_id` 均可公开断言，legacy 全局 key 只允许一次性安全迁移到明确项目，不得在多个项目复制；A 中启动命令或附件上传后切到 B，延迟结果只能回写 A 的 origin session，B 零变化；foreign `ChatAttachmentRef.project_id` 必须过滤或拒绝；刷新与主区/右 Dock/底 Dock 往返不破坏当前项目 context。
- workspace-manuscript-facade-read：创建 TeX document，写入 `paper.tex` 与 `main.bib`，随后通过 project-scoped generic Workspace list→tree→nodes read；两文件均返回 200、原字节/version/hash，不能因 generic store 无 `ws_doc_*` row 返回 404。缺失文件才 404；generic workspace 的真正缺失文件仍返回 null/404，不得错误 fallback。tree 节点 public size 为有限 `size`，客户端对 undefined/NaN/Infinity/负值统一显示 `0 B`，页面不得出现 `NaN undefined`。同一真实 BFF/Kernel 路径回归并覆盖 readVersion/blob backend resolution。
...
- intake-scan-static：`POST .../scan` 复算每文件服务端 sha256（篡改→422 stage_corrupted）+ 静态安全扫描（扩展名白名单/可执行 deny→rejected、HTML/SVG/无扩展名→quarantined、magic 校验不匹配→quarantined、ELF/PE magic→rejected、静态 secret 模式（private_key/AWS/GitHub/OpenAI）→quarantined 且 secret value 永不回显）+ **受控 archive 解包扫描（已实现，commit 98243ff）**：clean 的 .zip/.tar/.tar.gz/.tgz（扩展名+magic）逐条目复用 workspace 路径安全规则（绝对/`..`/NUL/盘符/反斜杠、重复规范化与大小写冲突、symlink/hardlink/设备/FIFO → quarantine `archive_special_entry`/`archive_path_invalid`/`archive_duplicate_path`）与炸弹上限（条目 ≤`ARCHIVE_MAX_ENTRIES`=1000、总解压 ≤`ARCHIVE_MAX_TOTAL_BYTES`=512MiB、单文件 ≤`ARCHIVE_MAX_FILE_BYTES`=64MiB、压缩比 >`ARCHIVE_MAX_RATIO`=100 → quarantine `archive_bomb`/`archive_file_too_large`）；展开视图写入 scan_result.archive_extract（status=ok + entries）与 scan_summary.extracted_entries/extracted_bytes；bz2/xz/7z 与单文件 gzip → status=unsupported（不 quarantine）；解包只写 intake 隔离区（零权威写不变，见 intake-preaccept-zero-authority）；scan_summary 如实记录 av_available=false；observations trust 固定 observed_unverified；
...
- 剩余（如实记录，属浏览器视觉/后续轮次）：拖拽/向导 Intake UI 的浏览器视觉验收（向导 DOM 已接线——client/modals/intake.ts 真实 begin→stage→scan→grill→propose→adopt，2026-08-11；视觉/拖拽/真实上传交互待 Playwright 类环境）、quarantine/scan 进度展示、分块 offset/hash 恢复上传（服务端整文件 staged ≤32MiB，UI 如实不做分块；恢复 = 已 staged 文件续传/重传幂等）、merge 冲突 keep/current/import/rename 交互、BFF /v2+accept 面与 Agent tool 面（begin/stage/scan/grill/propose/status）。**archive 解包扫描与 TeX/CodeSnapshot 采用物化已实现（服务端，commit 98243ff；evidence 见 intake-scan-static/intake-adopt-atomic 两行 + tests/unit/intake-materialize.test.ts 15/15）**；物化的浏览器展示（receipt import_mappings/CodeSnapshot 结果呈现）随向导视觉验收。**GUIDE-01 投影的 Intake/Grill 覆盖动作已落地（2026-08-11）**：kernel next-action.ts `intake_resume`/`intake_scan`/`intake_answer`/`intake_propose`/`intake_adopt`（`NextActionContext.intakes` 可选输入、projectProjection 从 listIntakes 投影、终态零动作），client `intakeGuidance` 把 next_actions_v2 的 intake 动作并入向导每步引导列表。

### 8.2 Trajectory 与 Subagent Topology

**standalone 投影与拓扑 API 层已实现（TRAJ-01/SUBAGENT-01 kernel/服务端轮，tests/unit/trajectory.test.ts 12/12、migration 0013 SCHEMA_VERSION 12、research-schemas `trajectory.ts`、research-kernel `trajectory.ts`、BFF child 路由解析；commit e298cee——见 hardening-v0.2-status.md §3 TRAJ-01/SUBAGENT-01 行）**：
...
- 剩余（如实记录，属 UI/DSH 集成轮）：浏览器拓扑渲染/进入 child/breadcrumb 导航、trajectory/topology 的 SSE 实时流（after_seq replay/gap/revoke——**服务端已实现（commit 98243ff，api-contracts.md §22）**：trajectory/stream keyset 增量 + workspace watch/stream + pty frames/stream，tests/unit/sse-streams.test.ts 7/7 + run-sse-tests.sh 67/67；浏览器事件源消费观感 NOT_RUN_MANUAL_PENDING）、10k node 浏览器 DOM 虚拟化与键盘/ARIA/zh-en 验收（Playwright 类环境不可用）、token 四桶/cost/permissions/retention 详情（需 DSH session adapter）、research_panel 插件侧调用 registerChildLink/updateChildState 接线（本轮 kernel/服务端已就绪，插件代码未动）。
...
- `/help`、`/list`、`/status`、`/gates`、`/jobs`、`/claims` 等文档和 UI starter 命令均有真实 handler，不落入 generic help；
...
- `/api/chat/survey` 必须在调用 connector 和写 corpus 之前执行认证 Principal membership 与可写角色检查；unknown/foreign project 返回 404，viewer/auditor 返回 403，且 Corpus Snapshot/Outbox/Project revision 计数不变；请求过大返回 413，不退化为 invalid_json；connector 的 citation_edges 必须进入 snapshot，每源 `source_status` 按“任一失败=pending、全部成功=complete”聚合，部分失败不得写成 complete；SCOPED 成功路径还断言 status=SURVEYING、next action=idea_generate；
...
- `readme-product-usage-only`：根 README 只含产品定位、使用边界、仍在开发中的显著提示、安装启动、Init/Resume/Upload、一级 slash command、工作台使用、Panel Dock 和可选开发集成；实现矩阵、测试计数、commit 证据与详细差距只链接到 hardening/acceptance/USAGE，不在 README 重复维护。
...
- next-action-v2-cards-rendered：Overview 渲染结构化 NextAction v2 卡——三态 tone（ready/blocked/done）与禁用规则（done 灰显、blocked 有 required 缺口禁用、ready 高亮）、label 按 i18n key 翻译或原样显示 kernel label、required 缺口列表（已知缺口 code 翻译/未知原样）、route/白名单 interaction 进入真正可完成动作的界面（gates/chat/runs/evidence/manuscript/budget 直达，ideas/contracts/release/overview 收敛 Overview tab，未知 route 安全回退）、`code='unknown'` 只读无 CTA、legacy `next_actions: string[]` 回退与畸形 v2 安全退化。`survey_run` 是专门交互：SCOPED 卡必须进入当前项目 Chat 并预填 `/survey <Brief.problem>`，不得进入空 Runs、不得自动发送；成功发送后 corpus snapshot 与 `SCOPED→SURVEYING` 同事务完成——tests/unit/next-action-cards.test.ts 35/35、tests/unit/next-action.test.ts 17/17、tests/unit/standalone-web.test.ts 18/18、tests/security/run-standalone-http-tests.sh 253/253（gui-plugin-plan.md §5.1，审计报告 §4 #11，2026-08-12 修复）。
...
- 自动证据必须绑定当前 worktree/commit、实际执行命令、环境、测试数与非零退出语义；历史计数、被 command substitution 吞掉的错误、只打印 FAIL 但最终 exit 0、未配置凭据却返回 PASS 均不算证据。
- 读取私有 DSH checkout 的本地 link 只能用于 typecheck/开发 fixture；固定私有 `@deepseek-ai/*` clean install、全新 DSH_HOME 和真实 host boot 单独记录，缺凭据时只能是 `NOT_RUN_MANUAL_PENDING`。
- 任何新场景必须同时更新负责规范、hardening 状态和人工验收队列；真实环境未运行时保持“已实现未验收”或更低状态。
...
- `project-delete-after-archive`：未归档项目 DELETE 得 409 `project_not_archived`；归档后只有 PI 可用 exact name + reason + expected_revision 删除，错误确认 422、旧 revision 409、非 PI 403、非成员/跨项目 404。成功返回稳定 DeletionReceipt，list/get/projection/所有项目写面均不再可见，刷新 UI 后 active project 清空并给出下一步；同 request id 重放同 receipt。审计中保留 `project.deleted`、Principal、request id，重启后 tombstone 不复活；另一个项目引用同一 sha256 时删除/后续 GC 不破坏其字节，released Bundle/retention hold 不被绕过。
- `project-delete-archived-browser-i18n`：Delete 只出现在 archived 项目的收缩操作区；确认框要求键入精确项目名并展示不可逆可见性/retention 边界，zh/en 即时切换无缺 key；取消零写入，成功后选择相邻项目或回 Start，Agent tool/command 中无 delete。
- `dsh-private-registry-install`：执行 §9.0 的真实私有包安装与 Cordis 生命周期；当前无私有 registry/credential/固定 Scholar 安装 spec 的开发环境记 `NOT_RUN_MANUAL_PENDING`。
- `ui-sidebar-dock-all-panels`：逐一把 chat/phase/gates/runs/artifacts/evidence/budget/manuscript/terminal/trajectory/topology/workspace/pty 从主区打开到右侧 Dock、移动到底部、打开回主区并关闭；任何时刻同页只有一个活实例。右侧/底部互换不得重建 DOM，Chat 草稿/附件、Workspace 与 TeX dirty/选中项/PDF、Terminal/PTY/Trajectory 的游标与滚动位置保持；主区/Dock 切换后 Terminal/PTY 以 after_seq、Workspace 以 after_revision、Trajectory 以最后 event seq 续接且无重复。拖动 separator 及方向键/Home/End resize，刷新恢复 panel/position/两尺寸，损坏 storage 回默认；1024/720/640 px 下 Project Sidebar 与 Panel Dock 不混淆、right 在窄屏仅视觉投影 bottom；zh/en、theme、ARIA orientation/value、focus 恢复全部通过。代码侧纯模型已由 `dock-layout.test.ts` 6/6 覆盖；真实浏览器 DOM、焦点、草稿和实时流观感记 `NOT_RUN_MANUAL_PENDING`。
...
- gate-human-bff-principal：Approve/Reject/Revise 的浏览器请求只带业务字段并进入 `/bff/research/gates/{gate_id}/decision`；BFF 从会话注入完整 Principal + request_id，再映射 Kernel Decision。测试必须断言请求 body 缺客户端 principal/actor 仍成功、伪造身份被忽略、直打 Kernel 缺 principal 仍 422；UI 对 `principal_required`/403/404/409/422/network 显示真实稳定分类，不得统一为“桥接错误”；已决 provenance 的 approved/rejected/revised 也必须走 zh/en 字典，未知未来枚举才原样显示。Human BFF 必须在解析/预检之前生成 request_id；bearer、CSRF/Origin、gate id、成员/角色、payload、Kernel unavailable/拒绝等每一条非 2xx 路径均断言 `error.code/message/request_id` 嵌套 envelope，浏览器 parser 必须保留该 request_id。**2026-08-12 已修复**：UI 单项/批量决策统一使用 Human BFF 且 payload 零身份字段；BFF 校验 bearer+CSRF+Origin、解析 gate→project、fresh membership/PI-operator role、注入 durable Principal/session 与服务端 request_id；客户端伪造 actor/principal/session/request_id 被丢弃；上游稳定 error code/request_id 保留，UI 分为身份、权限、资源、冲突、校验、网络六类。服务端在 Human BFF 路由入口即生成 request_id，预检与 Kernel 错误共用同一 envelope。证据：`gate-human-bff.test.ts` 5/5、`standalone-web.test.ts`、`kernel.test.ts` 共 154/154；`run-standalone-http-tests.sh` 247/247，包含仅 `{decision}` 正向 200、session ledger 对账和 forged identity 负向；本地构建实例对不存在 Gate 的无副作用冒烟返回 404 `project_not_found` + 服务端 request_id。
...
## 22. Name-only Init、Chat Grill、批量上传与 OCR/Provider 强制场景

- `init-name-only-no-gate`：POST /v2/projects 仅 name，返回 DRAFT/collecting + active Intake + membership/budget，Gate 数为 0；幂等重放同对象，异 hash 409；collecting projection 不出现 scope gate CTA，orchestrator 不自动补 Gate。**代码侧已闭环（2026-08-12，INIT-GRILL-02）**：kernel `createProjectForGrill` 单事务（DRAFT+collecting+PI membership+默认 Budget+active Init Intake，零 Gate）、v2 路由强制 Idempotency-Key/Principal（422 fail-closed）、同 key 同 hash 幂等重放/异 hash 409、v2 ZodError→422 validation_error（name 去空白 1–120）、`brief_status='collecting'` NextAction 只出 intake_resume/intake_answer 覆盖动作、orchestrator engine `decideActions(DRAFT, …, 'collecting')=[]` 且 engine 级 pollOnce 零 Gate 写。证据：tests/unit/kernel.test.ts INIT-GRILL-02 组（v2 HTTP 全链 7 用例）、tests/unit/orchestrator.test.ts engine 级 collecting 零 Gate 用例。
- `chat-grill-one-at-a-time`：每次 GET/answer 只暴露一个当前问题；revision stale 409；edit/skip/unknown 可恢复；刷新/重连从服务端继续；未来 OCR 候选必须带 source/page/confidence 且不自动成为 human_assertion。**Grill 代码侧已闭环（2026-08-12）**：确定性状态机（`PROJECT_GRILL_QUESTIONS` 固定 7 题顺序 + question_revision=1 + required），answer 记录 Human Principal/human_assertion，`/new`、`/confirm-brief` 一级 slash command，消息只保存 attachment/stage ref。OCR 请求面（`/v2/intakes/{id}/ocr-requests`）仍未实现，不得记为 `NOT_RUN_MANUAL_PENDING`。证据：kernel.test.ts Grill 顺序/确认用例、chat-command.test.ts、intake.test.ts（intake taxonomy 确定性）。
- `grill-confirm-scope-gate`：必答项处理后返回 Brief preview；非 PI/Agent confirm 403 或无工具；PI 使用 expected revisions 确认后写 confirmed Brief 与唯一 Scope Gate；confirm 重放不重复 Gate。**代码侧已闭环（2026-08-12）**：confirm 路由加入 BFF/Kernel 双侧 PI-only 能力表（`/grill/confirm`，researcher/viewer/auditor → 403）、kernel `requirePiOnly` 二次复检、事务写 canonical Brief + `brief_status=confirmed` + 唯一 pending Scope Gate（重复 confirm 409 `scope_gate_exists`）。证据：kernel.test.ts INIT-GRILL-02 confirm 用例（非 PI 403 / PI 200 / 唯一 Gate / 重放 409）。
- `intake-batch-chunk-resume`：多文件队列按 8 MiB chunk；暂停/刷新/重连从 committed offset 继续；相同范围同 hash 重放成功，gap/不同内容/total 变化 409；finalize 重算完整 hash/size，失败零 IntakeArtifact；2 GiB 默认与 10 GiB 硬上限、并发预留不能绕过。**代码侧已闭环（2026-08-12，CHUNK-01）**：`/v1/projects/{id}/intake/{iid}/upload-sessions*`（begin/append/finalize/abort/list + 24h GC），Content-Range + X-Chunk-SHA256 协议（顺序追加/乱序 409 chunk_gap/重放 replayed=true/overlap 409/total 409/hash 422/超限 413/越界 422），finalize 流式重算（chunk_incomplete/hash/size 不一致 422 零 artifact，重复 finalize 幂等），配额事务性预留（默认 2 GiB、硬上限 10 GiB、413 upload_quota_exceeded），>32MiB 大文件 scan 流式 hash + 有界 head 静态扫描（深 archive 扫描 >256MiB 记录 too_large 如实标注），扫描前字节只在隔离 intake staging（零权威写）。client 队列状态机（hashing→queued→uploading→paused→finalizing→scanning→ready/quarantined/failed，offset/hash/retry/pause/resume）+ driveUpload 注入式传输全生命周期；composer 附件按钮/拖拽/粘贴接线（消息只保存 attachment/stage ref）。证据：tests/unit/chunked-upload.test.ts 31/31（服务端全生命周期 + HTTP 面 + client 模型 + fake transport 驱动）、intake.test.ts 32/32 不破坏。真实浏览器拖拽/粘贴观感与 2–10 GiB 文件记 `NOT_RUN_MANUAL_PENDING`（manual-acceptance.md §6）。
- `provider-secretref-ssrf`：global Provider CRUD revision CAS；严格 SecretRef 拒绝 value/token/password；列表/日志/Trajectory/Manifest 零 secret；非法 scheme、redirect、DNS rebinding/proxy 越界 fail closed；项目只能提交 provider/model ID。**代码侧已闭环（2026-08-14，MODEL-01/OCR-CONFIG-01）**：`/v1/providers*` CRUD + revision CAS（409 provider_revision_conflict）、SecretRef 严格 schema（value/token/password/credential 字段 422 secret_value_forbidden，本 kernel 从不存储 value）、显式无凭据与损坏凭据元数据分离、base URL 服务端校验（scheme https-only（loopback 白名单 http）、userinfo 拒绝、私有/保留 IP 与未 allowlist DNS 主机 SSRF fail closed）、浏览器响应 credential 只回显 metadata+available（零 secret 断言）、项目绑定只收 opaque provider/model ID（provider 存在+enabled、模型在目录、能力匹配、快照 provider_revision+config_hash、binding/provider 双 revision CAS、删 provider 清绑定），Provider 与 binding 写面在 BFF/Kernel 双层限制为 PI/Operator。连接期 redirect/DNS-rebinding 复检由尚未实现的模型客户端承担（如实记录）。证据：tests/unit/provider-registry.test.ts 16/16。真实私有 Provider/模型服务调用记 `NOT_RUN_MANUAL_PENDING`。
- `ocr-mineru-config`：Settings「模型与 OCR」必须始终提供 MinerU 配置项；新建时固定 `provider_id=mineru`、`kind=mineru`、官方 API `https://mineru.net/api/v4`（仅允许规范化末尾 `/`）及 `flash/pipeline/vlm` OCR model 目录，编辑使用 provider revision CAS。Kernel 拒绝 MinerU URL 的 userinfo、非 443 端口、其他 path、query 与 fragment。Flash 允许无 credential；配置 SecretRef 时只发送 scheme/name/version/scope，响应只返回 metadata+available，禁止 Token/value/password，HTTP 明文字段拒绝使用稳定 `secret_value_forbidden`。活动项目选择模型时只 PUT `purpose=ocr/provider_id/model_id/expected_revision/expected_provider_revision`，不得携带 endpoint/credential；purpose 必须由所选 model 自身的 capability 满足，Provider 聚合 capability 不能放宽模型约束。zh/en、保存失败、无活动项目、读取失败、启用/禁用与 SecretRef 可用状态均可见；读取失败 fail closed，错误使用 `role=alert`/`aria-live`，所有 JSON/multipart 非幂等写零自动重试。Provider 写成功而 binding 写失败时必须显示部分成功并要求刷新重试。**代码侧已闭环（2026-08-14）**：Kernel 固定 MinerU 契约并要求 Pipeline/VLM SecretRef；Settings 可创建/编辑/启停、选择当前项目模型并携带双 revision；稳定错误只显示 zh/en 安全文案。自动证据：MinerU/Provider/API retry/UI Settings/i18n 定向集 62/62，整仓 90 文件 1274/1274，schemas/kernel build、根/UI typecheck、research-ui/plugin build、docs 21/21 均通过。该场景只关闭配置与 binding，不得把 OCR request/worker/provenance 误标为完成；真实浏览器保存观感仍为 `NOT_RUN_MANUAL_PENDING`。
- `ocr-explicit-model-provenance`：无配置/禁用/无 capability 时稳定失败且不回退；异步状态可恢复，idempotency 重放；成功结果保存 source/page/confidence/model/config pin 的 observed_unverified；低置信度触发 Grill，不产生 Gate/Evidence。
- `init-model-upload-i18n`：上述所有 chrome、错误、aria 与下一步提示 zh/en parity；运行时切换不丢项目名、答案或队列。**代码侧已闭环（2026-08-12）**：chat 附件按钮/暂停/继续/重试/失败/暂存提示新增 shell namespace zh/en 全 key（localeParity 与 i18n-chrome 扫描保持绿）；创建弹窗 name-only 已是 zh/en。真实浏览器、2–10 GiB、私有 Provider 和 OCR 服务均记录 `NOT_RUN_MANUAL_PENDING`，通过前 hardening 只能写“已实现未验收”。

## 23. 论文复现、一级 slash command、Chat 附件、环境与 Session Terminal

- `slash-command-direct-only`：帮助/补全/descriptor 只提供 `/new`、`/reproduce`、`/confirm-brief` 等一级命令；DSH command list 不注册聚合 descriptor，standalone 也不解析或兼容旧聚合前缀；旧形式必须返回 unknown command，且不得进入 starter、Tab 补全、帮助、文档示例或 provenance；命令 description 在 zh/en 下按 locale key 即时求值，切换语言后不得残留冻结英文。**代码侧已闭环（2026-08-12 实现轮）**：`/reproduce` 为一级 direct descriptor（src/plugin/commands.ts DIRECT_COMMANDS，无聚合注册），handler 解析 `<doi|arxiv|paper-artifact-id>`（paperRefFromToken：DOI/arXiv 新旧式/`sha256:<hex>` artifact id，非法 token 返回明确错误）→ `createReproductionSpec` → 返回 spec_id + 下一步（确认计划/执行 attempt/持久化 Report 才计 pass）；不再提交通用 baseline Job；run-dsh-plugin-tests.sh 55/55 命令注册不破坏；standalone parser 对旧聚合形式返回 unknown command，与帮助/补全/文档示例/新 provenance 同一 direct-only 契约（contract §1）。
- `chat-attachment-intake-only`：按钮/拖拽/粘贴多文件进入当前 active Intake 分块队列；消息只含 attachment ref；scan/adoption 前 Project Artifact/CodeSnapshot/Job/Evidence 零写；刷新恢复队列，zh/en/aria 完整。**代码侧已闭环（2026-08-12，CHUNK-01 逻辑层）**：composer 附件按钮 + 拖拽/粘贴 handler → `enqueueFiles`→hash→分块队列（`driveUpload` 注入式传输，begin/append/finalize/abort，pause/resume/retry/shouldContinue 暂停语义）→ 消息 `ChatMessage.attachment` 只存 ref（upload/intake/project/file/state，无字节）；队列 chip 显示 hashing/queued/uploading/paused/scanning/ready/failed + 百分比；服务端 pre-accept 零权威写（intake.test.ts ZERO authority 组）。证据：tests/unit/chunked-upload.test.ts client 模型组 + chat-command.test.ts。浏览器拖拽/粘贴真实观感、断网重连、zh/en/aria 视觉记 `NOT_RUN_MANUAL_PENDING`（manual-acceptance.md §6/§7 MANUAL-CHAT-ATTACH-SLASH）。
- `reproduction-spec-provenance`：DOI/arXiv/PDF、CodeSnapshot exact commit/submodules、data/license/split/preprocess、claims/metric locator、Contract 与 Principal 全部固定；raw repo URL 不能代替 Snapshot。**代码侧已闭环（2026-08-12 实现轮）**：PaperReproductionSpec 固定 paper_ref（DOI/arXiv/扫描 PDF Artifact，外部 metadata/full text untrusted）、owner Principal、claims 非空、git CodeSource 只接受 exact commit（branch/tag 422）、snapshot CodeSource 校验注册表存在且同项目（跨项目 422 code_snapshot_foreign）、data 固定 Artifact/hash 或 recipe+expected_sha256（clean-room 无法满足→blocked 不静默跳过）、execution binding 只接受 opaque 已登记 runner profile + target_id；外部 repo URL 不物化为 CodeSnapshot 前 spec 只是 draft（tests/unit/reproduction.test.ts 校验矩阵）。
- `reproduction-environment-pinned`：Spec/attempt/ExecutionPlan/RunManifest 固定 profile/target/environment revision/hash；local Docker 与 remote SSH 使用同一接口；客户端 endpoint/SSH credential/ProxyCommand 拒绝；unknown/offline/draining/cap mismatch 无本机回退。**代码侧已闭环（2026-08-12 实现轮）**：EnvironmentLock（image digest 仅 `@sha256:` 或 `sha256:` 形式、os/arch、runtime lock hash、CUDA/GPU、dataset/code hashes、runner profile/target revision+hash、effective config hash、network/resource policy、tool versions、SBOM ref）+ ExecutionBinding（opaque runner_profile_id/target_id，SSH host/port/user/key/jump host/mTLS 只存服务端 Settings/SecretRef，不进入 Spec/Job/argv/Bundle）；attempt 固定 spec revision + environment_pins + code_snapshot_id + data_pins；profile 未登记→422 runner_profile_unknown；真实远端执行/offline 回退语义复用既有 RUN-REMOTE-01 接口（本行剩余真实 SSH/mTLS 验收 NOT_RUN_MANUAL_PENDING）。
- `reproduction-comparison-report`：expected=0、absolute/relative、NaN/Infinity、unit/direction/aggregation、missing/duplicate metric、table row/column、figure data hash、manuscript TeX/PDF required/negative；exit 0 但 out-of-tolerance 报告 fail/inconclusive，不把 NextAction 标 done。**代码侧已闭环（2026-08-12 实现轮）**：research-kernel `reproduction-compare.ts` 纯比较器——allowed=max(absolute, abs(expected)*relative)；expected=0 时 relative 恒 0 由 absolute 决定（zero-safe）；NaN/Infinity、缺失（required fail/optional inconclusive）、重复 metric、unit/direction/aggregation 不匹配一律 fail；direction 不是误差替代；table 按稳定 row/column key 比较（missing/extra row/column、cell mismatch）；figure 先比 data hash（缺 expected hash→inconclusive），visual similarity 仅附加诊断不能翻转；manuscript 必须重建 TeX/PDF 且结构/文本/font/页数检查，缺输入→inconclusive 绝不 skipped-pass；`evaluateReportStatus`（required 全 pass→pass、任一 required fail→fail、仅 inconclusive→inconclusive、preflight blocked→blocked）+`suggestFailureClass`（metric_mismatch/table_mismatch/...，绝不 code_error）；ReproductionReportInput zod refine 拒绝 status/checks 不一致协议体；NextAction `baseline_reproduce` 只有持久化 Report status=pass 才 done（fail/inconclusive/blocked 报告→reproduction_retry_or_repair，绝不 done）；报告不可变进 CAS、行存 hash/ref（tests/unit/reproduction.test.ts 33/33）。
- `reproduction-attempt-report-fencing`：Spec revision、attempt idempotency、Job/Run/lease generation、signed RunManifest、verifier identity、immutable Report 与跨项目 AuthZ；Agent 无 report accept/Gate Decision。**代码侧已闭环（2026-08-12 实现轮）**：updateSpec revision CAS（409 reproduction_revision_conflict）；attempt idempotency（同 key+hash 重放同 attempt、异 hash 409）；startReproductionAttempt 固定 spec revision、generation=1、lease token 明文只回一次（hash 落库，STORE-06 模式）；report 提交按 generation+lease token hash fencing（409 lease_stale）且要求 verifier service principal（403 service_identity_required）+ SERVICE_ROUTES 的 x-service-token（403 service_token_required）；Report 不可变（CAS 内容寻址，行存 body_hash+cas_ref，读回复算 hash）；spec/attempt/report 全部 project-scoped（跨项目 404 防枚举，memberOr404 先解析 project）；Agent 工具面无 report accept/Gate Decision（本任务未新增任何 Agent 决策工具）。
- `session-multi-pty-binding`：每个 Operator/Research/Chat/Subagent context 可列出/打开多个 PTY；context A 切换到 B 后输入不会发送到 A；server-derived child/session、owner、lease expiry、expected generation、exact-parent/stale/cross-project 全负向；深链恢复正确标签。
- `fleet-clock-determinism`：Fleet 测试中的注册时间、heartbeat、offline 窗口与服务端 `now` 必须来自同一可注入时钟；fresh Agent 对无 Kernel Bearer 的 claim 稳定返回 401/non-retryable，stale Agent 稳定返回 409/retryable 且不得访问 Kernel。测试不得把固定历史 `last_seen` 与真实 `Date.now()` 混用，否则离线 admission 会遮蔽目标鉴权路径。
- 真实 Docker、远端 SSH/mTLS、多机断线、GPU、论文私有数据、TeX/PDF、真实浏览器附件和多 terminal 观感记 `NOT_RUN_MANUAL_PENDING`；通用 baseline Job/release script 不能计完整复现 PASS。
- workspace-crash-recovery-idempotent：workspace 的磁盘字节与 SQLite 元数据（`workspace_nodes` + `workspace_ops` ledger）必须由同一恢复协议收敛——对每个 fs/DB 边界注入崩溃并验证幂等恢复：rename 后 row 未更新（磁盘新字节+旧 row → 前滚修复 v+1/新 hash/补 op）、row 指向缺失字节（二进制从 CAS 恢复；文本本版本在 history → delete 前滚完成且 undo 保留；无任何副本 → `workspace_inconsistent` 隔离且重启持久、恢复字节后重扫自愈）、孤儿 `.ws-tmp-*` 清理（被 row 覆盖的同名文件不误删）、move 两窗口（未落库 rename 回滚 re-associate / 目标 row 已插入前滚完成）、double-scan 状态收敛一致。**代码侧已闭环（2026-08-11，hardening §5 P2 STORE-01/WORK-01）**：`scanWorkspaceIntegrity()`（kernel 构造期自动运行 + 按需；`workspaces.quarantine` 持久化隔离列，migration 0018 引入时 SCHEMA_VERSION 16、当前 0019 后为 17；隔离 workspace 读写 503 `workspace_inconsistent`）；证据：tests/unit/crash-recovery.test.ts 11/11（全窗口 + 幂等双扫 + 重启持久 + 自愈）、tests/security/run-workspace-tests.sh 48/48（新增 ws-crash-recovery 3 断言：真实 kernel 进程 kill+重启同 dataDir——启动隔离 503、恢复字节自愈、rename-before-row 前滚 v2；新增 ws-content-search 7 断言：行号/snippet 命中、大小写、二进制跳过、空 q 422、q+prefix/mode 冲突 422、路径搜索不破坏）、根 pnpm test 866/866；剩余：真实 kill -9/双进程/长期 recovery drill（NOT_RUN_MANUAL_PENDING，队列见 manual-acceptance.md §4）；fleet 侧结论见 hardening §5 P2 行（registry/assignment/spool 内存态，按 lease 过期语义自愈，不引入大持久化框架）；
