...
自然语言 Chat 是 project-scoped turn adapter，不新增 `/research` 聚合 descriptor，也不改变上述 direct command Registry。Standalone composer 接受任意普通文本并根据当前 Kernel projection 返回阶段感知回答；状态/想法/Gate/Jobs/Claims 等只读意图执行 canonical operation，用户明确要求且权威 NextAction ready 的无参数 `/write`、`/review`、`/release-bundle` 可直接执行，ready survey 还必须带明确 query。实验、复现或其他参数不完整写动作只返回可编辑 direct command。blocked、歧义、权限不足与 Human-only 决策零写；对话执行后重新读取 projection 并给出下一步，不得退化成 slash-only 或 `Unknown command`。
