...
`/new <name> [<brief-json>]` 是 name-only Init 引导入口，只有 `name` 必填；`brief-json` 省略时通过 DSH internal create+link 事务直接创建 `DRAFT/collecting` 空项目与 Init Intake，不得创建 Scope Gate，也不得返回 Brief 缺失错误。DSH `parseCommand()` 会在 `rawInput` 中保留命令名后的分隔空白，Scholar command adapter 必须在自身语法边界归一化该空白，并以真实的 `rawInput=" <name>"` 形状做回归测试。命令成功后，`dsh Scholar` 页签顶部的 Host-owned session projection 必须在下一次可见态轮询（最长 4 秒）显示同一 project id/name/status/brief status；该确认不得依赖下方 standalone 自己的项目列表或 dataDir。Grill 回答与 PI `/confirm-brief` 目前属于 authenticated standalone Human Chat/BFF 面，不冒充 Agent command。Agent Intake tool 只操作 observation/question/proposal，不提供 accept/adopt/merge-confirm 或任何 Gate Decision，最终 Adoption 与 Brief confirm 只能由 Human PI 完成。
...
根 package 的 dsh.bundle.patch 只插入 Agent `research-plugin` 行；根 manifest 另声明 `dsh.client` 与 `./client`，由 DSH Web 模块宿主加载配置卡和 `conversation.view` 页签。页签是 standalone 的 iframe/新页面启动器，不插入第二个 BFF 或业务状态层。standalone UI 独立管理 loopback server、BFF Token 与 BFF dataDir，但 Kernel sidecar 默认复用插件的 `127.0.0.1:7412` 与 `~/.dsh/research-kernel`；复用前仍须通过 endpoint identity 校验。只有一个 sidecar 实际拥有 Kernel 子进程，另一个必须 identity-verified reuse，禁止并发打开第二个业务数据库。
