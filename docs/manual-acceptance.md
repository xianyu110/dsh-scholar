...
- 环境：Headless Chrome 1920×1080，不使用证书绕过；DSH `https://210.34.241.17/`，Scholar `https://210.34.241.17:8443/`，当前用户 NSS DB 已信任经指纹核验的本地 CA。
- 故障注入：浏览器保持在同一 DSH session 和 `dsh Scholar` 页签，短暂停止 standalone 后端，使 8443 返回 502。结果为失败 iframe 已卸载（iframe 数 0），出现中文 `role=alert` 及“重试 / 在新页面打开 / 恢复本机默认地址”，未出现大灰块。
- 恢复：恢复 standalone，在不改变 URL 的情况下点击“重试”。结果为新 iframe `https://210.34.241.17:8443/` 可见（1632×652），HTML/client.js 均 200、失败 alert 清除；`/dsh-scholar-view/session-projection` 被调用，未关联 session 显示明确下一步提示。
- Token 负向：提交错误 Token 后显示 `Invalid token`，没有 `[object Object]`。
- 自动证据：整仓 94 files / 1359 tests、standalone HTTP 268/268、DSH plugin 55/55、全包 build、docs 22/22 通过；exact parent `targetOrigin`、RPC authority split、失败态 abort 与 retry listener 接线均有定向回归。
- 保持 `NOT_RUN_MANUAL_PENDING`：URL 恢复动作、真实 Host Chat provider/超时取消、父页导航 TOCTOU、zh/en、1280/720/640 px、键盘、屏幕阅读器与人工视觉验收尚未全部执行。
