...
2. DSH：Agent plugin、subagent follow-up、Cordis self-mod 隔离与 lifecycle；
   ...
   - **Latest DSH + current artifact smoke（2026-08-13）**：重新加载交互 shell 后，权限 0600 的临时 npm userconfig 可认证并从 npmjs.org 解析、安装精确 `@deepseek-ai/dsh@0.0.1-rc.2`；DSH realpath 位于全新 launcher。当前根插件正式 build + pack 得到 `@dsh-scholar/research-plugin@0.1.0`（tgz sha256 `494c39c1724e5548ced04735061aa6e1f92c25214d28fbd5e04a4c93f14aa8de`），但经公开 `dsh plugin --profile web add --save-exact <tgz>` 安装时，registry 对未发布的 `@dsh-scholar/research-client`、`research-kernel`、`research-schemas`、`scholar-connectors` 与其传递依赖 `analysis-worker` 返回 404；所以“单个当前产物可安装”结果为 **FAIL（发布物不自包含）**，不得记 artifact smoke PASS 或 published-registry PASS。诊断隔离方面，将上述五个内部包也分别 build + pack，并在临时 profile 以 tgz override（没有 checkout realpath/symlink）安装后，插件与四个直接运行时依赖均为 profile 内 `0.1.0`，profile dump 含 `research-plugin` 与 `kernel.port=0`，真实 DSH Web/Cordis 存活 8 秒、sidecar 物化 `kernel.db`，SIGTERM dispose 为 0。该诊断仅证明最新版 DSH host 与当前完整包集合的运行时兼容；关闭发布阻断必须选择并落实一种正式发布形态（先发布全部内部包并固定可解析版本，或把必要内部运行时打入根插件），随后从空 profile 只安装一个 Scholar spec 重跑。凭据未进入 argv/报告，临时目录在记录 hash 后删除。
