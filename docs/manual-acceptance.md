...
1. 浏览器：Init/Resume/Upload/Grill、Workspace、Interactive Terminal、TeX live preview、Trajectory/Subagent Topology、Settings/i18n；
   ...
   - **Interactive Terminal 视觉项（PTY-01，hardening §5 P1 行，acceptance-tests.md §21 `interactive-terminal-browser`）——PTY TUI client 逻辑层已实现（commit 98243ff）**：More →「PTY 终端」面板（#tab=pty 深链）——open 表单（workspace/preset/cwd/cols/rows + 钉定 profile/target）→ 会话工具栏（resize、INT/TERM/KILL、detach/reconnect、close）→ 纯文本输出区（gap/retention 截断标记、exit 行）→ 状态行（状态/in-out seq/掩码 lease+过期/generation/字节数/frames 消费 stream 状态（`pty.stream.*`）、idle TTL/lease 过期/权限撤销 close-reason 提示、lease 失效错误提示重新打开）——证据 tests/unit/pty-client.test.ts 43/43。**client SSE 消费已实现（commit 98243ff）**：frames 经 `SseClient` 消费 `GET /v1/pty/sessions/{id}/frames/stream?after_seq=`（frame/gap/exit/heartbeat；流失败回退 after_seq 轮询）。剩余浏览器验收（全部记 `NOT_RUN_MANUAL_PENDING`，Playwright 类环境不可用）：真实终端渲染（ANSI/xterm 类）、键盘输入、resize 拖拽、完整日志下载与窄屏/断线观感；SSE frames 流浏览器事件源观感（client 消费已实现，观感仍 NOT_RUN_MANUAL_PENDING）。
