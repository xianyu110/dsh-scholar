...
|---|---|---|---|
| GOV-01 | 认证 Human Principal durable | 已实现未验收 | HTTP gate decision 路由 fail-closed:decisionSchema.principal 必填,裸 actor→422 validation_error(kernel 内部 decideGate 保持 orchestrator 兼容);principal/tenant/auth_method/session 持久化并在 listDecisions 重读完整保留;BFF 注入登录身份(loopback operator),客户端不可伪造。证据:tests/security/run-gate-tests.sh 15/15(新增 Test 9 裸 actor 422 + 带 principal 200)、tests/unit/kernel.test.ts durable principal 用例;真实 Principal resolver(外部 IdP)未接,当前为 loopback operator 映射,待验收 |
...
| RUN-01 | 正式 Docker/快照/fencing/Manifest | 部分 | secure kind 拒 subprocess、approved Contract 绑定、严格 lease fencing、MetricsFileV1、data_artifact_ids 同项目+hash 校验、Run attempt(runs 表 claim 时写入、complete 时 finalize manifest/signature/finished_at,attempt_no 递增)、快照资源上限已实现(RUN-01a/b/c + 补充);仍缺默认必签 Manifest(当前为项目/kernel opt-in require_signed_manifest) |
...
| SEC-UI-01 | standalone token/Origin/CSRF/AuthZ | 已实现未验收 | CSRF session token(GET /api/session/csrf,32B hex,写路由强制 x-csrf-token,Origin 检查保留为第二层);/api/chat/survey 在 connector 与 corpus 写入前执行 membership,unknown/foreign→404 且 Corpus/Outbox 计数不变;--no-token 仅 loopback,非 loopback 组合 listen 前失败;token 经 0600 token 文件传递,日志/argv 无 secret;稳定错误码不泄露内部路径。证据:tests/security/run-standalone-http-tests.sh 119/119(含 CSRF 401/403、survey 404 fail-closed 计数不变、argv/log 扫描);修复:测试清理只杀自身实例(pkill 范围限定),start-standalone-ui.sh 相对路径在聚合器 cwd 下失效已改绝对路径;尚未绑定 CI job 报告,待验收 |
