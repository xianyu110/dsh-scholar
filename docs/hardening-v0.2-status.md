...
| API-01 | v2 + BFF AuthZ | 已实现 | v2 适配器落地(api-contracts §4):POST /v2/projects(Idempotency-Key 必填、同 key 同 hash 重放返回同一 project+initial Scope Gate+budget+membership、异 hash 409)、GET /v2/projects(cursor/limit 键集分页、默认 50 上限 200、x-principal-id 成员过滤)、GET /v2/projects/{id}、projection+capabilities(roles/membership/runner_profile)、gate-requests(拒绝携带 decision/actor/principal)、transitions(Gate 状态 422);0007 迁移持久化项目 idempotency_key/request_hash(SCHEMA_VERSION 6);standalone 代理透传 Idempotency-Key/X-Request-Id 并转发 /v2;HTTP 验收 40/40;单测 4 项;多 Principal 登录解析仍属 roadmap |
