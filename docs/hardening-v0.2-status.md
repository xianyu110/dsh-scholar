...
| STAT-01 | 单一正式分析实现 | 部分 | paired analysis 基础存在；formal metrics identity（MetricsFileV1 强制见 RUN-01c）与 lower-is-better（见 STAT-01a）已实现；仍缺 minimum_n、固定 10,000 resamples、canonical output 和 Analysis→accepted Evidence 链的完整验收 |
