...
| SELFMOD-01 | dev-only Cordis self tools | 已实现未验收(静态否定部分) | production 静态否定:research-plugin tarball/lib/src 不含 cordis_inspect/mount/unmount/tool-cordis/dump-config 字符串;production 配置不挂载 tool-cordis;dev overlay 仅 start-selfmod-dev.sh 且需 DSH_SCHOLAR_ENABLE_SELFMOD opt-in;依赖图不含 @deepseek-ai/dsh-tool-cordis;verify-docs.mjs 对 SELFMOD-01 违规 fail-closed。证据:tests/security/run-selfmod-tests.sh 15/15(已接入聚合器);动态 overlay inspect/mount/unmount 需外部 DSH host fixture,属 CI-01 待办,未在本仓库验收 |
