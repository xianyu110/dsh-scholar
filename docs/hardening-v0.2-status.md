...
| UI-02 | i18n zh/en | 基本具备 | locale adapter + zh/en 字典(common/shell/standalone/terminal/manuscript)+ 设置语言切换与即时重渲染已实现;manuscript 工作台与 tab 组/标签全部走 t();独立解锁页首帧前选择 locale+<html lang> 动态化;Playwright 验证 zh 解锁页/manuscript/tab 全量生效;各视图内部面板文案仍有少量硬编码,随 UI-04 拆分继续迁移 |
