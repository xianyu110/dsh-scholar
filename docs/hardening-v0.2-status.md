...
| API-01 | v2 + BFF AuthZ | 部分 | 基础落地:0006_project_members 表 + creator PI 种子(createProject.creator_principal_id)、list/add/remove 成员 API(member_manage 需既有 PI、最后 PI 不可删除)、/v1/projects/:id/members 路由、project.membership.updated 事件;单测 2 项;BFF 路由级 membership/AuthZ 强制与 v2 迁移、多 Principal 登录解析属 roadmap |
