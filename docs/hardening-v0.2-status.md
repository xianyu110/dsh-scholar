...
| API-01 | v2 + BFF AuthZ | 基本具备 | 基础:0006_project_members + creator PI 种子 + 成员 API(最后 PI 保护)+ /v1/projects/:id/members 路由 + 事件;BFF 强制已落地:standalone --principal 映射 loopback 操作者身份,project-scoped /v1 路由先查 kernel 权威 members(缓存),非成员与未知项目一律 404(防枚举),GET /v1/projects 列表按成员过滤;HTTP 验收 33/33 含 10 项 membership 用例;v2 路由/多 Principal 登录解析仍属 roadmap |
...
| UI-02 | i18n zh/en | 基本具备 | locale adapter + zh/en 字典 10 命名空间(common/shell/standalone/terminal/manuscript/overview/runs)+ 设置语言切换与即时重渲染;manuscript/tab/解锁页/Overview(pipeline 标签、下一步、快捷视图、预算/想法/合同/审计、%完成)/Runs(标题、筛选 chips、多选)全部走 t();Playwright 验证 zh 全量生效;Artifacts/Evidence/Budget 视图内部文案仍硬编码,随 UI-04 拆分迁移 |
