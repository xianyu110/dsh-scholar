...
DSH Host 模型桥是 natural turn 的可选回答 adapter，不是 mutation BFF。standalone HTML 的 CSP `frame-ancestors` 只允许 loopback，子页只在 origin-only `document.referrer` 为不同 origin loopback 时才能发送投影；Host 只允许来自配置 standalone origin 的 exact iframe source，远程 HTTPS/同 origin 嵌入均不开放模型 RPC。桥请求只允许 `{text,locale,project:{project_id,name,status,brief_status,next_actions_v2},history}` 的有界安全投影；Host 忽略客户端提供的 provider、model、principal、工具、endpoint 和 credential 字段，并从 Harness LLM registry 与插件配置解析模型。每 iframe 最多一个 in-flight request，已完成 request-id 保留有界 128 项去重窗口，20 秒超时/卸载/URL 切换通过 `AbortSignal` 取消 provider 调用。响应只允许 `{assistant_text,suggested_command?}`；`suggested_command` 必须是已注册一级 direct descriptor 且不是 Human-only command，浏览器只预填不执行。bridge unavailable/timeout/model failure 返回 adapter unavailable，由 standalone 使用本地 deterministic guidance；Token、原始模型错误、prompt、reasoning 和 provider 响应不得进入浏览器或普通日志。
