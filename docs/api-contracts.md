...
DSH Host 模型桥是 natural turn 的可选回答 adapter，不是 mutation BFF。standalone HTML 的 CSP `frame-ancestors` 仅允许配置的精确 HTTPS origin 或 loopback HTTP origin；子页只在 origin-only `document.referrer` 证明父页为不同 origin 的 loopback，或为同主机、不同端口/origin 的 HTTPS Host 时才能发送投影。Host 只允许来自配置 standalone origin 的 exact iframe source；跨主机 HTTPS、远端明文 HTTP 与同 origin 嵌入均不开放模型 RPC。桥请求只允许 `{text,locale,project:{project_id,name,status,brief_status,next_actions_v2},history}` 的有界安全投影；Host 忽略客户端提供的 provider、model、principal、工具、endpoint 和 credential 字段，并从 Harness LLM registry 与插件配置解析模型。每 iframe 最多一个 in-flight request，已完成 request-id 保留有界 128 项去重窗口，20 秒超时/卸载/URL 切换通过 `AbortSignal` 取消 provider 调用。响应只允许 `{assistant_text,suggested_command?}`；`suggested_command` 必须是已注册一级 direct descriptor 且不是 Human-only command，浏览器只预填不执行。bridge unavailable/timeout/model failure 返回 adapter unavailable，由 standalone 使用本地 deterministic guidance；Token、原始模型错误、prompt、reasoning 和 provider 响应不得进入浏览器或普通日志。

父子页必须从配置或 origin-only referrer 得到不可变的 exact origin；双向 `postMessage` 都必须用该值作为 `targetOrigin` 并同时校验 exact `WindowProxy`、origin、协议版本/request-id，禁止 `'*'`。ready timeout、iframe error/失败态、URL 变化、重试或卸载必须取消旧 Host Chat RPC，迟到结果不得写入新 iframe。DSH Connection 的 `/dsh-scholar-view` 使用 `trusted-host` authority 且只暴露 `chat-turn`、`session-projection`；loopback-only `/dsh-scholar` 单独承载 settings snapshot/mutation 与显式 Token Clipboard，两个 handler 必须拒绝对方的 endpoint，不能仅靠客户端隐藏。
...
### Connection RPC `/dsh-scholar-view`: `session-projection`

请求 `{session_id: string}`，接受 DSH Connection 的 `trusted-host` authority，session id 不做宽松 trim，必须原样匹配上述安全 opaque-id 语法。未关联返回 `{linked:false, session_id, stages:[…]}`；已关联返回 `{linked:true, session_id, project, stages, next_action, summary}`，字段集合与上述工具的安全投影一致。该 RPC 只读且不接受 project id；它是同一受信 DSH deployment 的 UI 视图传输，不是跨租户授权边界，真正的 mutation 仍只接受 Host 绑定的 Agent session。Host 在读取 projection 后再次核对 session link；不一致重试一次后返回 unavailable。handler 不暴露 Kernel/provider/standalone token、SecretRef、原始日志、prompt、tool args/result 或 transcript，并拒绝 settings/token endpoint；内部错误返回稳定 `Scholar session projection is unavailable`，不透传路径或上游响应。客户端必须以精确 allowlist 校验 root/project/stage/NextAction/summary/jobs，不允许额外字段；响应 session 精确相等、十阶段固定顺序、完整 NextAction/jobs 结构，`counts` 是可为空的动态字典但每个值都必须为非负有限整数；session 切换的 AbortSignal 必须传到底层 fetch。
