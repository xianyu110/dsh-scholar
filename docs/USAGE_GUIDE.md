...
在 DSH 自带 Chat 里可以直接用自然语言说“开始研究这个主题”“继续调研”“现在到哪一步了”或“下一步做什么”，无需先记住 slash command。Harness 会调用 Scholar 的受控会话入口：没有关联项目时提示 `/new <项目名>`；有关联项目时读取同一 session 的权威状态，明确回显识别到的研究动作、是否已执行、最新下一步与阻断。当前只有已就绪且用户以“开始/继续/执行”等正向动作词明确要求的调研快照可由这次对话直接执行；“聊聊研究方法”“不要调研”等讨论、歧义或否定话术不会写入。Gate、Brief 确认、外部研究采纳和发布决定仍需在 Scholar 中由人完成，生成想法等其他写动作会生成可编辑的 slash command。
