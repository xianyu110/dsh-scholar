...
当前仓库已有 Kernel、CAS、Runner（LocalDockerAdapter + RemoteRunnerAgent wire/fleet）、Analysis Worker、Durable Orchestrator、Connectors、DSH Agent 插件和独立原生 DOM UI。DSH 浏览器嵌入面已从目标和交付面删除。截至 2026-08-11 修复轮：ResearchOnboarding（begin→stage→scan→grill→propose→PI adopt 全链 + Agent prepare-only 工具）、结构化 NextAction（`next_actions_v2` + legacy 派生）、通用 Workspace（磁盘 adapter + 内容搜索 + crash 恢复 + tree client 逻辑层）、Interactive PTY（LocalPtyAdapter 真实 tty + TUI client 逻辑层）、远端 Runner（ExecutionPlan 签名 + fleet wire + 内存 spool 自愈语义）、Config Registry（7 scope 55 键 + pin + Settings 动态生成）、Trajectory/Subagent projection（双 lane 投影 + exact-parent 拓扑 + SSE 流）均已实现（代码侧）；剩余项全部为真实环境/浏览器验收（mTLS 证书链、真实远端 sandbox、Remote PTY、浏览器视觉项——队列见 manual-acceptance.md §4，记 NOT_RUN_MANUAL_PENDING）与明确标注的后续阶段（SecretRef 存储层、config 写面、job-scope 键、PTY idle TTL/retention registry 键见 execution-runtime.md §6.2）。其余 v1 路由、Human Principal、Evidence provenance、最终式日志、TeX workspace 和 i18n 证据见 hardening-v0.2-status.md。
