...
> 物化边界（commit 98243ff）：§6.1 的 TeX/CodeSnapshot 物化**在事务提交后**执行（见 §6.1 注记），不属于本事务的原子范围——事务内的 Workspace 映射指权威 Artifact/采纳对象，workspace 物化是事务后的尽力增强，失败记 gap/审计且不回滚 adopt（崩溃恢复按 adoption_id 重驱仍只保证事务内对象不部分采用）。
...
生产状态（如实记录）：`upload_hash_mismatch`/`upload_offset_conflict` 属于尚未实现的分块 Content-Range staged upload 协议（api-contracts.md §16 剩余项），当前代码不产生（`upload_offset_conflict` 已登记为 retryable 集合成员）；`scan_timeout` 尚无超时机制（静态扫描同步且有界，≤32 MiB/archive 上限内 CPU 有界）；archive 上限违规（条目数/总字节/单文件/压缩比）当前以扫描判定面表达——`scan_result.archive_extract.status=rejected` + `archive_bomb`/`archive_file_too_large`/`archive_unsupported_*`/`archive_special_entry` 等 code 并 quarantine，而非 HTTP `archive_limit_exceeded`（后者为协议级保留码）。其余 10 个码均由 kernel/adopt 路径产生并有测试证据（tests/unit/intake.test.ts、intake-materialize.test.ts）。
