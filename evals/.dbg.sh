...
echo "ANALYSIS status: $(curl -s -o /tmp/ana-body -w '%{http_code}' -X POST "http://127.0.0.1:$KPORT/v1/projects/$PROJ/analysis" -d '{"metric":"macro_f1"}' | node -e "let d='';process.stdin.on('data',c=>d+=c).on('end',()=>{const a=JSON.parse(d);console.log(JSON.stringify({artifact:a.artifact_id,mean:a.mean,effect:a.effect_size,ci:[a.ci_low,a.ci_high]}))})")
