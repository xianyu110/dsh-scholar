...
describe('explicit migrations', () => {
  afterEach(() => {
    ...
    while (MIGRATIONS.length > 5) MIGRATIONS.pop()
  })
  ...
  it('bumps a fresh database to SCHEMA_VERSION with all steps recorded', () => {
    ...
    expect(applied.map(r => r.id)).toEqual(['0001_schema_v2_initial', '0002_import_legacy_v1', '0003_terminal_tex_i18n_capabilities', '0004_artifact_media_type', '0005_code_snapshots'])
  ...
  it('imports a legacy v1 fixture: data preserved and upgraded in place', () => {
    ...
    expect((db.prepare('SELECT value FROM meta WHERE key = ?').get('schema_version') as { value: string }).value).toBe('4')
    // Projects preserved.
    ...
    db.close()
    const db2 = openDatabase(path)
    expect((db2.prepare('SELECT COUNT(*) AS n FROM schema_migrations').get() as { n: number }).n).toBe(5)
    expect((db2.prepare('SELECT value FROM meta WHERE key = ?').get('schema_version') as { value: string }).value).toBe('4')
    db2.close()
    rmSync(path, { recursive: false, force: true })
  })
