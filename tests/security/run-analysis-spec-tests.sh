...
APPROVE=$(api -X POST "$BASE/v1/projects/$PROJ/contracts/$CT/approve" -d '{"actor":"analysis-spec-eval"}' | jfield '.status')
...
SNAP=$(api -X POST "$BASE/v1/projects/$PROJ/code-snapshots" -d "{\"path\":\"$WORK/fixture\",\"description\":\"analysis-spec fixture\"}")
